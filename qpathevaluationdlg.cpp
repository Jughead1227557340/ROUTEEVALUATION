...
void QPathEvaluationDlg::SortPeratoSet()
{
    ...
    for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
    {
        qDebug() << "The " << i << "th routes normalized Routelength is :" << m_ParetoOptimalSet.at(i)->m_routeLength;
        qDebug() << "The " << i << "th routes normalized Consumption is :" << m_ParetoOptimalSet.at(i)->m_MobileConsumption;
        qDebug() << "The " << i << "th routes normalized survivability is :" << m_ParetoOptimalSet.at(i)->m_survivability;
        qDebug() << "The " << i << "th routes normalized matchup is :" << m_ParetoOptimalSet.at(i)->m_matchingupLevel;
    }
    //将各属性标准化,并添加到列表中
    //对每个属性而言，值越大越差
    for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
    ...
    //Calculate the dominated-or-not relationship between each two solutions for every objective
    qDebug() << "***开始计算两两航迹之间的关系***";
    for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
    {
        for (int j = 0; j < m_ParetoOptimalSet.size(); j++)
        {
            ...
            {
                //step_1, calculate the direction vector of the line from j to i
                ...
                //when a certain attribute of solution i is smaller than that of solution j, it is possible that solution i is dominated by solution j for this objective
                //The first type of situation about RouteLength
                if (m_ParetoOptimalSet.at(i)->m_routeLength < m_ParetoOptimalSet.at(j)->m_routeLength)
                {
                    ...
                    double AngleOfLength  = asin(directionVector.length * 1.0 / sqrt(tempVariable));
                    ...
                    if (AngleOfLength < thresholdAngleOfLength)
                    {
                        ...
                        double APD_Length = thresholdAngleOfLength - AngleOfLength;
                        double temp = (RouteLength.at(i) - RouteLength.at(j)) / (RouteLength.at(0) - RouteLength.at(RouteLength.size() - 1));
                        double PIR_Length = 1.0 / (temp * temp);
                        m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore += APD_Length * PIR_Length;
                    }
                }
                if (m_ParetoOptimalSet.at(i)->m_MobileConsumption < m_ParetoOptimalSet.at(j)->m_MobileConsumption)
                {
                    ...
                    //step_2, calculate the included angle based on the formula
                    double tempVariable = directionVector.length * directionVector.length + directionVector.consume * directionVector.consume + directionVector.survive * directionVector.survive + directionVector.match * directionVector.match;
                    double AngleOfConsume  = asin(directionVector.consume * 1.0 / sqrt(tempVariable));
                    ...
                    if (AngleOfConsume < thresholdAngleOfConsume)
                    {
                        ...
                        m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount++;
                        //calculate APD (Angle Penalty Degree) and PIR (Penalty Intensity Rate), respectively
                        double APD_Consume = thresholdAngleOfConsume - AngleOfConsume;
                        double temp = (MobileConsume.at(i) - MobileConsume.at(j)) / (MobileConsume.at(0) - MobileConsume.at(RouteLength.size() - 1));
                        double PIR_Consume = 1.0 / (temp * temp);
                        m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore += APD_Consume * PIR_Consume;
                    }
                }
                if (m_ParetoOptimalSet.at(i)->m_survivability < m_ParetoOptimalSet.at(j)->m_survivability)
                {
                    ...
                    //step_2, calculate the included angle based on the formula
                    double tempVariable = directionVector.length * directionVector.length + directionVector.consume * directionVector.consume + directionVector.survive * directionVector.survive + directionVector.match * directionVector.match;
                    double AngleOfSurvive  = asin(directionVector.survive * 1.0 / sqrt(tempVariable));
                    ...
                    if (AngleOfSurvive < thresholdAngleOfSurvive)
                    {
                        ...
                        m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount++;
                        //calculate APD (Angle Penalty Degree) and PIR (Penalty Intensity Rate), respectively
                        double APD_Survive = thresholdAngleOfSurvive - AngleOfSurvive;
                        double temp = (SurviveProb.at(i) - SurviveProb.at(j)) / (SurviveProb.at(0) - SurviveProb.at(RouteLength.size() - 1));
                        double PIR_Survive = 1.0 / (temp * temp);
                        m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore += APD_Survive * PIR_Survive;
                    }
                }
                if (m_ParetoOptimalSet.at(i)->m_matchingupLevel < m_ParetoOptimalSet.at(j)->m_matchingupLevel)
                {
                    ...
                    //step_2, calculate the included angle based on the formula
                    double tempVariable = directionVector.length * directionVector.length + directionVector.consume * directionVector.consume + directionVector.survive * directionVector.survive + directionVector.match * directionVector.match;
                    double AngleOfMatch  = asin(directionVector.match * 1.0 / sqrt(tempVariable));
                    ...
                    if (AngleOfMatch < thresholdAngleOfMatch)
                    {
                        ...
                        m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount++;
                        //calculate APD (Angle Penalty Degree) and PIR (Penalty Intensity Rate), respectively
                        double APD_Match = thresholdAngleOfMatch - AngleOfMatch;
                        double temp = (MatchLevel.at(i) - MatchLevel.at(j)) / (MatchLevel.at(0) - MatchLevel.at(RouteLength.size() - 1));
                        double PIR_Match = 1.0 / (temp * temp);
                        m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore += APD_Match * PIR_Match;
                    }
                }
            }
        }
    }
    qDebug() << "***两两航迹之间的关系计算完毕***";
    //Then, mornalize the DominatedTimesCount and TotalAnglePenaltyScore of each solution or route
    int maxDominatedTimesCount;
    int minDominatedTimesCount;
    double maxTotalAnglePenaltyScore;
    double minTotalAnglePenaltyScore;
    //求出DominatedTimesCount的最大最小值
    qDebug() << "求出DominatedTimesCount以及TotalAnglePenaltyScore的最大最小值";
    for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
    {
        if (i == 0)
        {
        ...
        else
        {
            ...
            else if (m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount < minDominatedTimesCount)
            {
                minDominatedTimesCount = m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount;
            }
            else
    ...
    //求出TotalAnglePenaltyScore的最大最小值
    for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
    {
        if (i == 0)
        {
        ...
        else
        {
            ...
            else if (m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore > maxTotalAnglePenaltyScore)
            {
                maxTotalAnglePenaltyScore = m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore;
            }
            else
    ...
    qDebug() << "DominatedTimesCount以及TotalAnglePenaltyScore的最大最小值计算完毕";
    qDebug() << "开始计算最终的得分";
    //接下来对每个解的属性标准化,并计算最终的分数
    if (m_ParetoOptimalSet.size() == 1)
    {
    }
    else
    {
        ...
        for (int i = 0; i < m_ParetoOptimalSet.size(); i++)
        {
            if(maxDominatedTimesCount ==  minDominatedTimesCount)
            {
                m_ParetoOptimalSet.at(i)->m_pareto.DominatedTimesCount = 1;
            }
            else
            {
            ...
            if (maxTotalAnglePenaltyScore == minTotalAnglePenaltyScore)
            {
                m_ParetoOptimalSet.at(i)->m_pareto.TotalAnglePenaltyScore = 1;
            }
            else
            {
    ...
    if (m_ParetoOptimalSet.size() >= 2)
    {
        ...
        {
            //find out the minimum value by each circulation
